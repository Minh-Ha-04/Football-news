                ...
                if (isMounted) {
                    ...
                    if (data.tags && data.tags.length > 0) {
                        try {
                            ...
                            console.log('Related articles:', relatedResponses);
                            ...
                            const allRelatedArticles = relatedResponses
                                ...
                                .filter(
                                    (relatedArticle) =>
                                        relatedArticle && relatedArticle._id && relatedArticle._id !== data._id,
                                )
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (isSaved) {
                ...
                await axios.post(
                    `${API_URL}/article/${article._id}/save`,
                    {},
                    {
                        headers: {
                            Authorization: `Bearer ${token}`,
        ...
        <div className={cx('news')}>
            ...
            <div className={cx('container')}>
                <div className={cx('left')}>
                    ...
                    <strong className={cx('desc')}>{article.description}</strong>
                    <div className={cx('article-image')}>
                        {article.image && <img src={`${API_URL}${article.image}`} alt={article.title} />}
                    </div>
                    <div className={cx('content')}>{article.content}</div>
                    <div className={cx('media')}>
                        <button className={cx('media-item', { saved: isSaved })} onClick={handleSave}>
                            <FontAwesomeIcon icon={faBookmark} />
                            {isSaved ? 'Bỏ lưu' : 'Lưu bài'}
